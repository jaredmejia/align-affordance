...
class ContentNet(BaseModule):
    ...
    # @torch.no_grad()
    def decode_first_stage(self, z, predict_cids=False, force_not_quantize=False, keep_grad=False):
        context_manager = torch.no_grad if not keep_grad else torch.enable_grad
    ...
    def forward(self, batch, keep_grad=False, num_steps_backprop=10):
        inp = self.get_input(batch)
        bs = len(inp['x0'])
        ...
        samples, sample_list = self.eval_diffusion.plms_sample_loop(
                    ...
                    intermed=True,
                    keep_grad=keep_grad,
                    num_steps_backprop=num_steps_backprop,
                )
        samples = self.decode_first_stage(samples, keep_grad=keep_grad)
        
        if not (keep_grad or sample_list is None):
            sample_list = [self.decode_first_stage(e) for e in sample_list]
